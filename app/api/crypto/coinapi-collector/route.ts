...
async function getCoinApiCollector(request: Request) {
  ...
  try {
    ...
    const paper = isCryptoCapabilityEnabled("paperMatchingEnabled")
      ? await matchCryptoPaperOrders()
        .catch(()=>({ok:false,processed:0,fills:0,providerRequests:0}))
      : {ok:true,status:"shelved",processed:0,fills:0,providerRequests:0};
    ...
    const paper = isCryptoCapabilityEnabled("paperMatchingEnabled")
      ? await matchCryptoPaperOrders().catch(()=>({ok:false,processed:0,fills:0,providerRequests:0}))
      : {ok:true,status:"shelved",processed:0,fills:0,providerRequests:0};
