...
export default function HomeReferenceSurface({
  ...
  const contenders = opportunities.slice(0, 8);

  return (
    <main className="htb-home" aria-label="HT Labs Home">
      <section className="htb-tape" aria-label="Market context" tabIndex={0}>
        {marketContext ? (
          <>
            ...
            {[
              ["SPY", marketContext.spy.change],
              ["QQQ", marketContext.qqq.change],
            ].map(([label, change]) => (
      ...
      <div className="htb-workbench">
        <aside className="htb-opportunities" aria-labelledby="htb-opportunities-title">
          <div className="htb-rail-heading">
            <h2 id="htb-opportunities-title">Opportunities</h2>
            <span>Live</span>
          </div>
          <ol className="htb-opportunity-list">
            {contenders.map((candidate, index) => {
              const active = candidate.ticker === opportunity.ticker;
              return (
                <li key={`${candidate.strategy ?? "canonical"}-${candidate.ticker}`}>
                  <button type="button" aria-pressed={active} onClick={() => onSelect(candidate)}>
                    <span className="htb-opportunity-rank">{String(index + 1).padStart(2, "0")}</span>
                    <span className="htb-opportunity-symbol">
                      <strong>{candidate.ticker}</strong>
                      <small>{candidate.relativeVolume > 0 ? `${candidate.relativeVolume.toFixed(1)}× RVOL` : readable(candidate.stage)}</small>
                    </span>
                    <span className="htb-opportunity-market">
                      <strong className={candidate.change >= 0 ? "is-positive" : "is-negative"}>{changeLabel(candidate.change)}</strong>
                      <small>{Math.round(candidate.opportunityScore)}</small>
                    </span>
                  </button>
                </li>
              );
          ...
          <Link href="/scanner" className="htb-rail-link">Open full scanner <span aria-hidden="true">→</span></Link>
        </aside>

        <section className="htb-primary" aria-labelledby="htb-symbol-title">
          <header className="htb-symbol-header" data-home-priority="1-ticker-price">
            <div>
          ...
          <div data-home-priority="3-chart">
            <HomeReferenceChart symbol={opportunity.ticker} />
          </div>
        </section>

        <aside className="htb-intelligence" aria-labelledby="htb-intelligence-title" data-home-priority="4-intelligence">
          <div className="htb-rail-heading">
            <h2 id="htb-intelligence-title">HT Intelligence</h2>
            <span className={eligible ? "is-positive" : "is-warning"}>{eligible ? "Qualified" : "Monitor"}</span>
          </div>
          ...
          <section className="htb-intel-section">
            ...
            {prox && prox.status !== "unavailable" ? (
              <>
                <strong>{readable(prox.pulse?.state ?? prox.status)}</strong>
                <p>{prox.event?.headline ?? "Bounded market evidence is attached to the canonical decision."}</p>
              </>
            ) : <p>No fresh Pro X evidence is attached.</p>}
          </section>

          <section className="htb-intel-section htb-agent-plan">
            <h3>Agent X</h3>
          ...
          <details className="htb-evidence">
            ...
            <div>
              {visibleEvidence.length > 0 ? (
                <ul>{visibleEvidence.map((signal, index) => <li key={`${signal}-${index}`}>{signal}</li>)}</ul>
              ) : <p>No additional evidence is available.</p>}
