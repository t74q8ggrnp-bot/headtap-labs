...
export default function MobileBeforeCrowdCard({
  ...
  return (
    <article className="ht-mobile-home-card mx-4 mb-3 flex-shrink-0 overflow-hidden" aria-labelledby={`mobile-before-crowd-${opportunity.ticker}`}>
      ...
      <div className="border-b border-white/8 px-5 pb-4 pt-3" data-home-priority="1-ticker-price">
        <div className="flex items-end gap-3">
          <h2 id={`mobile-before-crowd-${opportunity.ticker}`} className="font-mono text-[2.8rem] font-black leading-none tracking-[-0.06em] text-white">{opportunity.ticker}</h2>
        ...
        <p className="mt-3 text-sm font-bold leading-tight text-zinc-300">{opportunity.stage}</p>
          ...
          <div className="border-b border-white/8 px-5 py-4">
            ...
            <ul className="space-y-2">
              {opportunity.signals.slice(0, 4).map((signal, index) => (
                <li key={`${signal}-${index}`} className="text-xs font-semibold leading-5 text-zinc-400">{signal}</li>
              ))}
            </ul>
          </div>
        )}
