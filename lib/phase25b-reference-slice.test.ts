...
test("Phase 2.5B styles retain flat surfaces and exact reference proportions", () => {
  ...
  const chart = source("app/components/home/HomeReferenceChart.tsx");
  assert.match(chart, /landscapeQuery\.matches[\s\S]*viewportHeight - 164[\s\S]*mobileQuery\.matches[\s\S]*viewportHeight - 302[\s\S]*terminalQuery\.matches[\s\S]*terminalChartHeight[\s\S]*terminalChromeHeight[\s\S]*: 500/);
