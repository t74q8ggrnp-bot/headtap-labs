...
test("mobile Home presents Spot Momentum, a full-width 90-minute chart, then Pro X", () => {
  ...
  assert.match(card, /onPlanStateChange=\{setAgentPlanState\}/);
  ...
  assert.match(forming, /onPlanStateChange\?: \(state: HomeTradePlanState\) => void/);
  assert.match(forming, /view=trade_plans&symbol=/);
  ...
  assert.match(card, /Sign in to load your verified Agent X targets/);
  assert.match(card, /No verified Agent X targets are available/);
