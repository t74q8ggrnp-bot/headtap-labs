...
test("Home terminal wires real Canonical lists and keeps required actions explicit", () => {
  ...
  assert.match(markets, /href=\{`\/market\?ticker=\$\{encodeURIComponent\(symbol\)\}`\}/);
...
test("desktop chart controls render below the chart and expose only functional Draw controls", () => {
  ...
  assert.match(chart, /terminalQuery\.matches[\s\S]*terminalChartHeight[\s\S]*terminalChromeHeight/);
